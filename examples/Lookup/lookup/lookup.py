try:
	...
	import lookup.extensioncore		# non-"from" import gives more relevant error message (see uno.py)
	from lookup.extensioncore import *
except ImportError:
...
class Lookup( ComponentBase, XMouseListener, XActionListener ):
	SUPPORTED_LANGUAGES = SUPPORTED_LANGUAGES
	def firstrun( self ):
		self.addMenuItem( 'com.sun.star.text.TextDocument', '.uno:AVMediaPlayer', 'Look Up Selection', 'lookup' )

		# assign the Alt-L shortcut
		supplier = self.ctx.ServiceManager.createInstanceWithContext( 'com.sun.star.ui.ModuleUIConfigurationManagerSupplier', self.ctx )
		configmanager = supplier.getUIConfigurationManager( 'com.sun.star.text.TextDocument' )
		smgr = configmanager.getShortCutManager()
		key = uno.createUnoStruct( 'com.sun.star.awt.KeyEvent' )
		key.KeyCode = uno.getConstantByName( 'com.sun.star.awt.Key.L' )
		key.Modifiers = uno.getConstantByName( 'com.sun.star.awt.KeyModifier.MOD2' )
		smgr.setKeyEvent( key, self.commandURL( 'lookup' ) )
		smgr.store() 
	...
	def uninstall( self ):
		for t in documenttypes:
			try:    # some document types may not be available at all
				...
				debugexception()
		self.removeMenuItem( 'com.sun.star.text.TextDocument', 'lookup' )
	def lookup( self ):
		selection = self.getcontroller().Selection
		words = []
		for s in enumerate( selection ):
			if hasattr( s, 'String' ):
				words.append( s.String )
		if len( words ) == 0:
			return
		import urllib
		url = 'http://www.google.com/search?sourceid=openoffice&ie=UTF-8&q=' + urllib.quote( ' '.join( words ) )
		shellexec = self.ctx.getServiceManager().createInstanceWithContext( 'com.sun.star.system.SystemShellExecute', self.ctx )
		shellexec.execute( url, '', uno.getConstantByName( 'com.sun.star.system.SystemShellExecuteFlags.DEFAULTS' ) )
