...
class ComponentBase( SelfUpdating, unohelper.Base, XServiceName, XInitialization, XComponent, XServiceInfo, XServiceDisplayName, XJobExecutor, XJob ):
	...
	def loadlocalization( self ):
		self.localization = {}
		try:
			dir = 'DanielDarabos%sDialogs'%self.__class__.__name__
			import glob, os
	...
	def createdialog( self, dialogname ):
		...
		dlg = dlgprovider.createDialog( 'vnd.sun.star.script:DanielDarabos%sDialogs.%s?location=application'%(self.__class__.__name__, dialogname) )
