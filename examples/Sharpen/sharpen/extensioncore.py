...
class propset( unohelper.Base, XPropertySet, XServiceInfo ):
	...
	def getImplementationName( self ):
		return 'org.openoffice.eoec.PropertySet'
...
class ComponentBase( SelfUpdating, unohelper.Base, XServiceName, XInitialization, XComponent, XServiceInfo, XServiceDisplayName, XJobExecutor, XJob ):
	def __init__( self, *args ):
		...
		try:
			self.ctx = args[0]
			self.config = self.getconfig( 'org.openoffice.eoec.%sSettings/ConfigNode'%self.__class__.__name__, update = True )
			self.initpath()
			self.initlanguage()
		except:
			debugexception()
	...
	def getImplementationName( self ):
		try:
			return 'org.openoffice.eoec.' + self.__class__.__name__
		except:
			debugexception()
	def supportsService( self, s ):
		return s in self.services
	...
	def commandURL( self, command ):
		return 'service:org.openoffice.eoec.%s?%s'%(self.__class__.__name__, command)
	...
	def getServiceName( cls ):
		try:
			return 'org.openoffice.eoec.' + cls.__name__
		except Exception:
			debugexception()
