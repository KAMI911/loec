...
try:
	...
	def initpython( path ):
		...
		if sys.hexversion >= 0x02030000 and sys.hexversion <= 0x0203ffff:
			...
			debug( '%s only supports Python 2.3.x and Python 2.5.x.'%classname )
			raise Exception( '%s only supports Python 2.3.x and Python 2.5.x.'%classname )
		if path not in sys.path:
			sys.path.insert( 0, path )
			import idphoto_generatedsettings as gs
			if gs.DEBUG:
	...
	initenvironment( 'IDPhoto' )
	import idphoto.idphoto			# non-"from" import gives more relevant error message (see uno.py)
	from idphoto.idphoto import *
except:
	debugexception()
