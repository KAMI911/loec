...
class ComponentBase( SelfUpdating, unohelper.Base, XServiceName, XInitialization, XComponent, XServiceInfo, XServiceDisplayName, XJobExecutor, XJob ):
	def __init__( self, *args ):
		...
		try:
			self.ctx = args[0]
			self.config = self.getconfig( '%prefix%.%sSettings/ConfigNode'%self.__class__.__name__, update = True )
			self.initpath()
			self.initlanguage()
		except:
			debugexception()
	...
	def loadlocalization( self ):
		self.localization = {}
		try:
			dir = '%VendorName%%sDialogs'%self.__class__.__name__
			import glob, os
	...
	def commandURL( self, command ):
		return 'service:%prefix%.%s?%s'%(self.__class__.__name__, command)

	def createdialog( self, dialogname ):
		psm = self.ctx.ServiceManager
		dlgprovider = psm.createInstance( 'com.sun.star.awt.DialogProvider' )
		dlg = dlgprovider.createDialog( 'vnd.sun.star.script:%VendorName%%sDialogs.%s?location=application'%(self.__class__.__name__, dialogname) )
