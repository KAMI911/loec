...
try:
	...
	def getpath( ctx, classname ):
		config = getconfig( ctx, '%prefix%.%sSettings/ConfigNode'%classname )
		path = config.Origin
	...
	def initpython( path ):
		...
		if path not in sys.path:
			...
			import %extensionname%_generatedsettings as gs
			if gs.DEBUG:
	...
	initenvironment( '%ExtensionName%' )
	import %extensionname%.%extensionname%			# non-"from" import gives more relevant error message (see uno.py)
	from %extensionname%.%extensionname% import *
except:
	debugexception()
